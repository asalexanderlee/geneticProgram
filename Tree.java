...
public class Tree {
 ...
 public void mutate(String[] termSet, String[] funcSet){
  ...
  if (stringIsDigit(randNode.getData())){
	  System.out.println("Changing leaf that was: " + randNode.getData());
   randNode.setData((new Random()).nextDouble() < 0.45 ? termSet[new Random().nextInt(termSet.length)] : "x");
  }else{
	  System.out.println("Changing inner node that was: " + randNode.getData());
 ...
 public double getY(double x){
   ...
   if (myRoot.getLeft() == null && myRoot.getRight() == null){
	   //if the root contains a number, return number; 
	   //otherwise, it is a variable, so return the value of x
 ...
 public Tree treeCopy(){
	 Tree newTree = new Tree(treeCopy(myRoot));
	 newTree.setSize(mySize);
   return newTree;
 }
 
 //treeCopy helper
 public Node treeCopy(Node root){
    if (root == null) return null;
    Node copy = new Node(root.getData()); 
    copy.setLeft(treeCopy(root.getLeft()));
    copy.setRight(treeCopy(root.getRight()));
    return copy;
