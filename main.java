...
public class main {
	...
	public static Tree createSymbReg(List<Double> xValues, List<Double> yValues,String[] termSet, String[] funcSet){
		...
		for (int i = 0; i < population.length; i++){
			double fitness = Math.pow(population[i].getY(xValues.get(i)) - yValues.get(i),2);
			if (fitness < min){
				min = fitness;
				best = i;
			}
  ...
  public static double getFitness(List<Double> xValues, List<Double> yValues, Tree tree){
    ...
    for (int i = 0; i < xValues.size(); i++){
      double error = Math.pow(tree.getY(xValues.get(i)) - yValues.get(i),2);
      fitness += error * error;
    }
    return fitness;
  ...
  public static void main(String args[]){
		...
		String method = "full";
		createSymbReg(xValues, yValues, terms, possOperators);
